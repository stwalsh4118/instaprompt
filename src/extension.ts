...
export function activate(context: vscode.ExtensionContext) {
	...
	async function handleAddPrompt(): Promise<void> {
		try {
			...
			// Open webview editor for content
			openMarkdownEditor(context, {
				...
				onSave: async (content) => {
					try {
						...
						throw error; // Re-throw to prevent editor from closing
					}
				},
				onCancel: () => {
					// No action needed - editor closes automatically
				},
			});
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
	...
	async function handleEditPrompt(): Promise<void> {
		try {
			...
			// Handle name-only editing (keep existing flow)
			if (selectedEditOption.label === 'Name') {
			...
			// Handle content or both editing with webview editor
			let newName: string | undefined = undefined;
			if (selectedEditOption.label === 'Both') {
			...
			// Open webview editor with existing content
			openMarkdownEditor(context, {
				title: `Edit: ${selectedPrompt.name}`,
				...
				onSave: async (content) => {
					try {
						...
						const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
						vscode.window.showErrorMessage(`Failed to update prompt: ${errorMessage}`);
						throw error; // Re-throw to prevent editor from closing
					}
				},
				onCancel: () => {
